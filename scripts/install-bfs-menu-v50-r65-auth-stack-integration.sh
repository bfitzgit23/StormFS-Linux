...
configure_kernel_source_policy() {
        ...
        case "$KERNEL_PACKAGE_VALUE" in
                linux) src="$(find /usr/src -maxdepth 1 -type d -name 'linux-*-BFS-Linux' -printf '%T@ %f\n' 2>/dev/null | sort -nr | head -n1 | cut -d' ' -f2-)" ;;
                linux-lts) src="$(find /usr/src -maxdepth 1 -type d -name 'linux-*-BFS-LTS' -printf '%T@ %f\n' 2>/dev/null | sort -nr | head -n1 | cut -d' ' -f2-)" ;;
        esac
        [[ -n "$src" ]] || { echo "Could not find source tree for selected kernel flavor: $KERNEL_PACKAGE_VALUE" >&2; return 1; }
        ln -sfn "$src" /usr/src/linux
}
