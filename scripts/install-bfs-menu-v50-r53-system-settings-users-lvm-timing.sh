...
show_accounts_review() {
        ...
        if [[ -n "$USERNAME" ]]; then
                text+="Primary account: $USERNAME\n"
                text+="  Type: Standard user\n"
                text+="  Groups: Standard BFSOS groups ($STANDARD_USER_GROUPS)\n"
                text+="  Password: optional during installation; blank keeps login locked\n"
        else
                text+="Primary account: not configured\n"
        fi
        ...
        if ((${#ADDITIONAL_USERS[@]})); then
                ...
                for ((i=0; i<${#ADDITIONAL_USERS[@]}; i++)); do
                        ...
                        text+="  - ${ADDITIONAL_USERS[$i]} — $(account_type_label "$type")\n"
                        if [[ "$type" == service || "$type" == service-login ]]; then
                                text+="      Supplementary groups: ${groups:-none}\n"
                                if [[ "$type" == service-login ]]; then
                                        text+="      Login: interactive shell enabled; password remains optional\n"
                                else
                                        text+="      Login: non-interactive/locked by default\n"
                                fi
                        else
                                text+="      Groups: Standard BFSOS groups\n"
                                text+="      Password: optional; blank keeps login locked\n"
                        fi
                done
        else
...
rm -f "$STATE_DIR/last_failure" /run/bfs-install-failure
else
