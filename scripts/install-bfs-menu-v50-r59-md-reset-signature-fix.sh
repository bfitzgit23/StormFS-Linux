...
storage_reset_destroy_metadata() {
        ...
        # mappings, but deliberately keep active MD arrays assembled while the
        # destructive checklist is built. Array-level signatures such as LUKS
        # live on /dev/mdX itself and disappear from device discovery if the MD
        # array is stopped first.
        ...
        while IFS= read -r dev; do
                ...
                # While an MD array is active, offer the array device itself for
                # clearing array-level LUKS/LVM/filesystem signatures, but do not
                # offer its member partitions. Wiping member MD superblocks while
                # the array is assembled is unsafe and generally fails as busy.
                # To erase member MD metadata, use "Deactivate only" first, then
                # return to Destroy selected metadata with the array stopped.
                if is_active_md_member "$dev"; then
                        continue
                elif is_md_block_device "$dev"; then
                        detail="Active MD array; ${detail}"
                elif command -v mdadm >/dev/null 2>&1 &&
        ...
        done < <(
                {
                        ...
                        # Include every currently active MD array regardless of
                        # personality (raid*, linear/JBOD, or future MD types).
                        list_active_md_arrays
                        # Include all active MD members using sysfs topology rather
                        # than relying on lsblk FSTYPE or /proc/mdstat field layout.
                        list_active_md_members
                } | sort -u
        )
        ...
        while IFS= read -r dev; do
                ...
                if is_md_block_device "$dev"; then
                        # /dev/mdX represents the assembled array. Do not zero the
                        # member superblocks when the user selected only an old
                        # filesystem/LUKS/LVM signature on the array itself.
                        pvremove -ff -y "$dev" 2>/dev/null || true
                        if ! wipefs -a "$dev"; then
                                status=1
                        fi
                else
                        # Inactive member devices may safely have old MD metadata
                        # removed. Non-MD devices simply make zero-superblock a
                        # harmless no-op.
        ...
        if ((status != 0)); then
                dialog_message "Storage reset incomplete"                         "One or more selected devices still contain detectable metadata or could not be wiped.\n\nRe-scan with wipefs -n, lsblk, /proc/mdstat, and mdadm --examine before continuing."
                return 1
        fi
