...
compiler_build_settings_menu() {
        ...
        while true; do
                ...
                themed_menu choice "Compiler / Build Settings" \
                        ...
                        7 "Verify MD5/checksums: $VERIFY_MD5" \
                        8 "Verify signatures: $VERIFY_SIGNATURE" \
                        9 "Verify footprints: $VERIFY_FOOTPRINT" \
                        10 "Restore inherited/default build values" \
                        11 "Back"
                case "$choice" in
                        ...
                        6)
                                ...
                                ;;
                        7) [[ "$VERIFY_MD5" == yes ]] && VERIFY_MD5=no || VERIFY_MD5=yes ;;
                        8) [[ "$VERIFY_SIGNATURE" == yes ]] && VERIFY_SIGNATURE=no || VERIFY_SIGNATURE=yes ;;
                        9) [[ "$VERIFY_FOOTPRINT" == yes ]] && VERIFY_FOOTPRINT=no || VERIFY_FOOTPRINT=yes ;;
                        10) BUILD_JOBS=inherited; BUILD_OPT=inherited; BUILD_CCACHE=inherited; BUILD_CCACHE_SIZE=inherited; BUILD_TMPFS=yes; BUILD_TMPFS_SIZE=auto; VERIFY_MD5=yes; VERIFY_SIGNATURE=yes; VERIFY_FOOTPRINT=yes ;;
...
configure_pkgmk_tmpfs() {
        ...
        if [[ "$size" == auto ]]; then
                ...
                if ((gib < 16)); then
                        # Match the settings-screen recommendation: on small-RAM
                        # systems, "auto" means use the disk-backed work tree
                        # instead of silently creating a tiny tmpfs ceiling.
                        if [[ -f /etc/fstab ]]; then
                                sed -i '\|^[[:space:]]*tmpfs[[:space:]]\+/var/cache/pkg/build-work[[:space:]]\+tmpfs[[:space:]]|d' /etc/fstab
                        fi
                        log "RAM-backed pkgmk workspace auto-disabled (${gib} GiB RAM); using disk-backed /var/cache/pkg/build-work"
                        return 0
                elif ((gib < 32)); then
                        size=8G
                elif ((gib < 64)); then
