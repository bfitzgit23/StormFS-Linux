...
recover_saved_storage_stack() {
        ...
        # without a RECOVERY_MD record (notably Linear/JBOD, whose lsblk TYPE is
        # "md", not "raid*"). Refuse to skip that dependency silently.
...
list_luks_candidates() {
        ...
        while read -r p t sz fs mp; do
                ...
                [[ -z "${seen[$p]:-}" ]] || continue
                [[ "$t" == part || "$t" == raid* || "$t" == linear || "$p" == /dev/md* ]] || continue
                [[ -z "$mp" ]] || continue
