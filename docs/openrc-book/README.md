# BLFS OpenRC Book for StormFS Linux

A comprehensive guide to using OpenRC as the init system for StormFS Linux.

> **Based on:** the LFS/BLFS 13.1 release line for the StormFS Linux base, adapted for OpenRC; OpenRC is the default init/service path
> **Service integration:** OpenRC service scripts are supplied by the StormFS `openrc-init-scripts` port and documented throughout this book.

## Table of Contents

1. [Introduction to OpenRC](chapters/01-introduction.md)
2. [Installation](chapters/02-installation.md)
3. [Configuration](chapters/03-configuration.md)
4. [Runlevels](chapters/04-runlevels.md)
5. [Service Management](chapters/05-service-management.md)
6. [Init Scripts Reference](chapters/06-init-scripts.md)
7. [Writing Custom Init Scripts](chapters/07-custom-scripts.md)
8. [Migration from Systemd](chapters/08-migration.md)
9. [Networking](chapters/09-networking.md)
10. [Display Managers](chapters/10-display-managers.md)
11. [Audio Configuration](chapters/11-audio.md)
12. [Bluetooth](chapters/12-bluetooth.md)
13. [Remote Desktop](chapters/13-remote-desktop.md)
14. [System Services](chapters/14-system-services.md)
15. [Troubleshooting](chapters/15-troubleshooting.md)
16. [Advanced Topics](chapters/16-advanced.md)
17. [XFCE Desktop](chapters/17-xfce.md)
18. [KDE Plasma Desktop](chapters/18-kde.md)
19. [GNOME Desktop](chapters/19-gnome.md)
20. [LXQt Desktop](chapters/20-lxqt.md)
21. [Window Managers](chapters/21-window-managers.md)
22. [Web Browsers](chapters/22-browsers.md)
23. [Office Suites](chapters/23-office.md)
24. [Multimedia](chapters/24-multimedia.md)
25. [Graphics and Design](chapters/25-graphics.md)
26. [Development Tools](chapters/26-development.md)
27. [System Utilities](chapters/27-utilities.md)
28. [Communication](chapters/28-communication.md)
29. [BLFS Package Mappings](chapters/29-package-mappings.md)
30. [Rootfs Archives and Live ISO Images](chapters/30-rootfs-and-live-iso.md)

## About This Book

This book follows the BLFS (Beyond Linux From Scratch) style and provides detailed instructions for configuring and using OpenRC on StormFS Linux. It covers service installation, dependency ordering, runlevels, desktop integration, remote access, migration from systemd, and rootfs/live-ISO image construction while preserving systemd procedures as reference material where applicable. The Calamares installer is intentionally maintained outside this book.

## Conventions

- Commands that need to be run as root are prefixed with `#`
- Commands that can be run as a regular user are prefixed with `$`
- Configuration file paths are shown in `monospace`
- Important notes are highlighted in blockquotes
- Cross-references to other chapters are linked

## Contributing

To contribute to this book, please submit pull requests to the StormFS Linux repository.
