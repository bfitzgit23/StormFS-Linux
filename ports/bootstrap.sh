...
. /var/lib/pkgmk/extension" > $LFS/tmp/pkgmk.conf
	...
	for i in $basepkg; do
		if [ "$1" != rebuild ]; then
			...
			pkgadd -r   $LFS $_force  -f $(ls -1 $packagedir/$i#* | tail -n1) || { umountfs; exit 1; }
			case $i in
...
EOF
			...
			chroot $LFS env -i PATH=$LFSPATH prt-get update -im -fr -if -fi $i || { umountfs; exit 1; }
		fi
	done
	umountfs
...
basepkg="aaa_filesystem linux-headers man-pages glibc tzdata zlib bzip2 xz file ncurses readline m4 bc binutils libxcrypt gmp mpfr mpc attr acl gcc pkgconf libcap sed psmisc iana-etc bison flex pcre2 grep bash libtool gdbm gperf expat inetutils perl perl-xml-parser intltool autoconf automake openssl ca-certificates curl kmod gettext elfutils libffi sqlite python coreutils check diffutils gawk findutils groff less gzip zstd iptables libtirpc iproute2 kbd libpipeline make patch man-db tar texinfo python3-setuptools python3-pip python3-flit-core python3-wheel ninja meson util-linux linux-pam libcap shadow libpng which freetype fuse grub popt mandoc efivar efibootmgr grub-efi vim nano python3-markupsafe python3-packaging python3-tomli python3-pyproject-hooks python3-build python3-installer python3-pytz python3-babel python3-jinja2 systemd dbus procps-ng util-linux e2fsprogs libarchive pkgutils prt-get httpup  ports prt-utils signify"
